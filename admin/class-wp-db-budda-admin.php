...
class Wp_Db_Budda_Admin {
    ...
    public function enqueue_scripts() {
        ...
        if ('budda-booking_page_wp-db-budda-addnew' == get_current_screen()->id) {
            ...
            //to edit booking we should send extra parameters: booking ID and action from url
    ...
    /**
     * Validate data from form and if they are ok, insert new booking or update existing one in database.
     *
     * @since    1.0.0
     */
    public function add_new_booking() {
        ...
        if (sizeof($_POST) > 0 && isset($_POST[$this->plugin_name])) {
            ...
            //if data are invalid, show errors
            if (is_wp_error($myErrors) && !empty($myErrors->errors)) {
            ...
            } // data passed validation, insert or update them
            else {
                global $wpdb;
                ...
                $class = "updated";
                $message = __('Information has been successfully saved', $this->plugin_name);
                echo "<div class=\"$class\"> <p>$message</p></div>";
    ...
    /**
     * Define unavailable dates and previously chosen dates for current booking.
     *
     * @since    1.0.0
     */
    ...
    public function save_error(){
        file_put_contents(dirname(dirname(__FILE__)).'/includes/error_activation.txt', ob_get_contents());
    }
